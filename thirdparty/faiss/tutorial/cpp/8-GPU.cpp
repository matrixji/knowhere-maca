...
void create_index(
    ...
    if ((access(filename, F_OK)) == -1) {
        ...
        faiss::Index* ori_index = faiss::index_factory(
                d, index_description, faiss::METRIC_INNER_PRODUCT);
...
void execute_index(
    ...
    // index1 = faiss::gpu::index_cpu_to_gpu(&res, 0, &index_composition0,
    // &option0); delete index1; index1 = faiss::gpu::index_cpu_to_gpu(&res, 1,
    // &index_composition1, &option1); delete index1;
    ...
    for (long i = 0; i < 1; ++i) {
        ...
        std::thread t00(
                ...
                &option0,
                &index_composition0,
                std::ref(gpu_index_ptr00));
        //        std::thread t2(GpuLoad, &res, 1, &option1,
        //        &index_composition1, std::ref(gpu_index_ptr1));
        std::thread t01(
                GpuLoad,
                &res,
                0,
        ...
        GpuExecutor(
                ...
                k,
                xq);
        //        GpuExecutor(gpu_index_ptr1, res, 1, &option1,
        //        &index_composition1, nq, nprobe, k, xq);
    }
...
faiss::METRIC_INNER_PRODUCT); auto device_index =
faiss::gpu::index_cpu_to_gpu(&res, 0, ori_index);
...
"false"); printf("ntotal = %ld\n", device_index->ntotal);
    ...
    // Copy quantizer to GPU 0
    auto index1 = faiss::gpu::index_cpu_to_gpu(&res, 0, &index_composition0,
&option0); delete index1;
    ...
    // Copy quantizer to GPU 1
    index1 = faiss::gpu::index_cpu_to_gpu(&res, 1, &index_composition1,
&option1); delete index1;
...
//    t2.join();
    for(long i = 0; i < 10; ++ i) {
        ...
        std::thread t00(GpuLoad, &res, 0, &option0, &index_composition0,
std::ref(gpu_index_ptr00));
//        std::thread t2(GpuLoad, &res, 1, &option1, &index_composition1,
std::ref(gpu_index_ptr1)); std::thread t01(GpuLoad, &res, 0, &option0,
&index_composition0, std::ref(gpu_index_ptr01));

        t00.join();

        GpuExecutor(gpu_index_ptr00, res, 0, &option0, &index_composition0, nq,
nprobe, k, xq);

        t01.join();
//        t2.join();
        GpuExecutor(gpu_index_ptr01, res, 0, &option0, &index_composition0, nq,
nprobe, k, xq);
//        GpuExecutor(gpu_index_ptr1, res, 1, &option1, &index_composition1, nq,
nprobe, k, xq);
    }
