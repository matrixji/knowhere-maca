...
std::priority_queue<Node, std::vector<Node>, CompareByFirst> RHNSW::
        search_layer(
...
void RHNSW::searchKnn(
    ...
    std::priority_queue<Node, std::vector<Node>, CompareByFirst>
            top_candidates = search_base_layer(
                    qdis, ep, std::max(efSearch, k), dist, bitset);
